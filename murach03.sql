...
specified table.  The FROM clause specifies what table to use, in this case we'll use 
the Invoices table.  This is pretty much as simple as a query gets.  The SELECT 
clause specifies a list of expressions in terms of columns that we want to include 
in our query.  We found that its a good idea to explicitly specify which database 
we want to use with our query. Our text suggests we should always make sure to
